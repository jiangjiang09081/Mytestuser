...
- (void)addSeparatorLineWithLeftMargin:(float)leftMargin
    ...
    if (position & LinePositionTop) {
        if (!self.topSeparatorLine)
        {
            ...
            UIView *upHalfLine = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 1, 0.5)];
            upHalfLine.tag = 1221;
            UIView *downHalfLine = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 1, 0.5)];
            downHalfLine.tag = 1222;
            [self.topSeparatorLine addSubview:upHalfLine];
            [self.topSeparatorLine addSubview:downHalfLine];
        }
        
        [[self.topSeparatorLine viewWithTag:1221] setBackgroundColor:color];
        [[self.topSeparatorLine viewWithTag:1222] setBackgroundColor:color2];
        
        CGRect rect = {self.frameX + leftMargin, 0, self.frameWidth - leftMargin - rightMargin, lineHeight};
        self.topSeparatorLine.frame = rect;
        [[self.topSeparatorLine viewWithTag:1221] setFrame:CGRectMake(0, 0, rect.size.width, 0.5)];
        [[self.topSeparatorLine viewWithTag:1222] setFrame:CGRectMake(0, 0.5, rect.size.width, 0.5)];
    ...
    if (position & LinePositionBottom) {
        if (!self.bottomSeparatorLine)
        {
            ...
            UIView *downHalfLine = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 1, 0.5)];
            downHalfLine.tag = 1222;
            [self.bottomSeparatorLine addSubview:upHalfLine];
            [self.bottomSeparatorLine addSubview:downHalfLine];
        }
        
        [[self.bottomSeparatorLine viewWithTag:1221] setBackgroundColor:color];
        [[self.bottomSeparatorLine viewWithTag:1222] setBackgroundColor:color2];
        ...
        [[self.bottomSeparatorLine viewWithTag:1221] setFrame:CGRectMake(0, 0, rect.size.width, 0.5)];
        [[self.bottomSeparatorLine viewWithTag:1222] setFrame:CGRectMake(0, 0.5, rect.size.width, 0.5)];
                              ...
                              position:(LinePosition)position
{
    ...
    if (position & LinePositionTop)
    {
        ...
        [[self.topSeparatorLine viewWithTag:1221] setBackgroundColor:[UIColor clearColor]];
        [[self.topSeparatorLine viewWithTag:1222] setBackgroundColor:[UIColor clearColor]];
    ...
    if (position & LinePositionBottom)
    {
        ...
        [[self.bottomSeparatorLine viewWithTag:1221] setBackgroundColor:[UIColor clearColor]];
        [[self.bottomSeparatorLine viewWithTag:1222] setBackgroundColor:[UIColor clearColor]];
...
- (void)removeSeparatorLinePosition:(LinePosition)position {
    if (position & LinePositionTop) {
        [self.topSeparatorLine removeFromSuperview];
    }
    else {
        [self.bottomSeparatorLine removeFromSuperview];
    }
}
