...
- (void)addInfiniteScrollingWithActionHandler:(void (^)(void))actionHandler {
    ...
    if(!self.infiniteScrollingView) {
        ...
        // customView for mobileOa
        // SVInfiniteScrollingStateLoading // 正在加载
        // SVInfiniteScrollingStateTriggered
        // SVInfiniteScrollingStateStopped // 暂无更多
        // SVInfiniteScrollingStateAll
        // SVInfiniteScrollingStateCancel  // 暂无更多

        
        UILabel *loadingView = [self customLabelWithText:@"正在加载..." withFrame:view.bounds];
        [view setCustomView:loadingView forState:SVInfiniteScrollingStateAll];
        
        UILabel *stopView = [self customLabelWithText:@"" withFrame:view.bounds];
        [view setCustomView:stopView forState:SVInfiniteScrollingStateStopped];
        
        UILabel *cancelView = [self customLabelWithText:@"暂无更多" withFrame:view.bounds];
        [view setCustomView:cancelView forState:SVInfiniteScrollingStateCancel];
    }
}

- (UILabel *)customLabelWithText:(NSString *)text withFrame:(CGRect)frame {
    CGRect lblFrame = CGRectInset(frame, 10, 0);
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVInfiniteScrollingStateLoading && self.enabled) {
        ...
        if(!self.scrollView.isDragging && self.state == SVInfiniteScrollingStateTriggered)
        ...
        else if(contentOffset.y > scrollOffsetThreshold && (self.state == SVInfiniteScrollingStateStopped || self.state == SVInfiniteScrollingStateCancel) && self.scrollView.isDragging)
            ...
            self.state = SVInfiniteScrollingStateStopped;
        else if(contentOffset.y < scrollOffsetThreshold  && self.state != SVInfiniteScrollingStateCancel)
            self.state = SVInfiniteScrollingStateCancel;
    }
}
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(hasCustomView) {
        ...
        switch (newState) {
            ...
            case SVInfiniteScrollingStateCancel:
                [self.activityIndicatorView stopAnimating];
                break;
        }
    }
